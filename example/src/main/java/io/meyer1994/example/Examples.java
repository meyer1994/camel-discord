...
public class Examples {
  ...
  /**
   * Queries the 20 nearest labeled examples and returns weighted good/bad scores in the range [0.0,
   * 100.0]. Returns 50.0 for both when no examples exist.
   *
   * @param embedding comma-separated vector string, e.g. "[0.012, -0.034, ...]"
   */
  public Score getScore(String embedding) {
    String sql =
        """
        SELECT
            COALESCE(
            ...
            COALESCE(
                SUM(similarity * CASE WHEN label = 'bad' THEN 1.0 ELSE 0.0 END)
                / NULLIF(SUM(similarity), 0),
                0.5
            ) * 100.0 AS bad_score
        FROM (
    ...
    return jdbcTemplate.queryForObject(
        sql,
        (rs, rowNum) -> new Score(rs.getDouble("good_score"), rs.getDouble("bad_score")),
        embedding,
        embedding);
  }

  /**
   * Camel-friendly overload. Reads {@code variable.embedding}, computes the score, and writes
   * {@code variable.goodScore}, {@code variable.badScore}, and {@code variable.score} (all 0-100)
  ...
  public void score(Exchange exchange) {
    ...
    Score scores = getScore(embedding);

    exchange.setVariable("goodScore", scores.goodScore());
    exchange.setVariable("badScore", scores.badScore());
    exchange.setVariable("score", scores.goodScore());
  }

  private record Score(double goodScore, double badScore) {}
}
