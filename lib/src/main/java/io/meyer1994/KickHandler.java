...
public class KickHandler {
    ...
    public void onChatEvent(PusherEvent event) {
        try {
            ...
            consumer.getExceptionHandler().handleException(
                    "Could not parse Kick WebSocket message", exception);
        }
    }
    ...
    private void emitChatMessage(KickChatMessage event) {
        ...
        exchange.getMessage().setHeader(KickConstants.HEADER_CHANNEL_NAME,
                consumer.getEndpoint().getChannel());
        ...
        try {
            ...
            consumer.getExceptionHandler().handleException(
                    "Error processing Kick chat message", exchange, exception);
        }
    }
}
